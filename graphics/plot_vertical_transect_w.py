...
w_xsect = vertcross(w, z, wrfin=ncfile,
...
x = np.arange(w_xsect.shape[-1])
z_vals = to_np(w_xsect.coords["vertical"])

# w color fill
c = ax.contourf(x, z_vals, to_np(w_xsect), levels=30, cmap="viridis")
plt.colorbar(c, ax=ax, label="w (m/s)")
...
# Labels and title
ax.set_title("Vertical Cross Section of w with θ Contours\n(West to East, through %f)" % desired_lat)
...
plt.tight_layout()
plt.savefig("w_vertical_transect_with_theta.png", dpi=300)
plt.show()
